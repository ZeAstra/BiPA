...
default_hooks = dict(
    ...
    # custom_dual_optim=dict(type='BayesianOptimizationHook'),  
)
...
resnet_pretrain_ckpt_path = "pretrain/resnet50-19c8e357.pth"
# model settings
model = dict(
    type='USISAnchor',
    ...
    backbone=dict(
        ...
        resnet_config=dict(
            ...
            num_stages=4,  
            out_indices=(0, 1, 2, 3),  
            frozen_stages=4,    # USIS10K: freeze first 3 stages (stage 0,1,2,3 are frozen when frozen_stages=4)
            norm_cfg=dict(type='BN', requires_grad=False),  
            style='pytorch'  
        ),
        init_cfg=dict(
            vit_checkpoint=sam_pretrain_ckpt_path,  
            resnet_checkpoint=resnet_pretrain_ckpt_path  
        ),
        peft_config=None,  
    ),
    adapter=dict(
        ...
        use_space_adapter=False,   # Set to True for USIS10K
