...
work_dir = './work_dirs/stage2_finetune'

## ---------------------- MODEL ----------------------

crop_size = (1024, 1024)
num_classes = 7
# For foreground dataset, set num_classes = 1
...
# dataset settings
# Current: Multi-class dataset
# To switch to foreground segmentation dataset(USIS10K), modify:
...
# For multi-class dataset only, comment out for foreground dataset
...
# Add Bayesian optimization hook
default_hooks = dict(
    timer=dict(type='IterTimerHook'),
    logger=dict(type='LoggerHook', interval=20),
    param_scheduler=dict(type='ParamSchedulerHook'),
    checkpoint=dict(type='CheckpointHook', interval=3, max_keep_ckpts=3, save_best=['coco/bbox_mAP', 'coco/segm_mAP'],
                    rule='greater', save_last=True),
    sampler_seed=dict(type='DistSamplerSeedHook'),
    custom_dual_optim=dict(type='BayesianOptimizationHook'),  # Bayesian optimization hook
)


load_from="stage1_train.pth",   # load weights from stage1 training
