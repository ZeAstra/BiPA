...
class USISSamVisionEncoder(BaseModule):
    def __init__(
            ...
            embed_dim=1280,
            num_heads=8,
            iim_config: dict = None,  # 新增IIM配置参数
            loss_consistency: dict = dict(type='SmoothL1Loss', loss_weight=1.0)
    ):
        super(USISSamVisionEncoder, self).__init__(init_cfg=init_cfg)

        # 初始化 ViT 编码器
        sam_config = SamConfig.from_pretrained(hf_pretrain_name).vision_config
        if extra_config is not None:
            sam_config.update(extra_config)
        vision_encoder_vit = UAViTEncoder(sam_config)

        # 初始化 ResNet 编码器
        if resnet_config is not None:
            vision_encoder_resnet = ResNetx(**resnet_config)
        else:
            raise ValueError("resnet_config must be provided.")
        ...
        if init_cfg is not None:
            ...
            load_checkpoint(
                vision_encoder_vit,
                init_cfg.get('vit_checkpoint'),
                map_location='cpu',
                revise_keys=[(r'^module\.', ''), (r'^vision_encoder\.', '')]
            )
            load_checkpoint(
                vision_encoder_resnet,
                init_cfg.get('resnet_checkpoint'),
                map_location='cpu',
                revise_keys=[(r'^module\.', ''), (r'^vision_encoder\.', '')]
            )
        ...
        if peft_config is not None and isinstance(peft_config, dict):
            ...
            self.vision_encoder_vit = get_peft_model(vision_encoder_vit, peft_config)
            if is_main_process():
                self.vision_encoder_vit.print_trainable_parameters()
        else:
            self.vision_encoder_vit = vision_encoder_vit

        # 保存 ResNet 编码器
        self.vision_encoder_resnet = vision_encoder_resnet
    ...
    def forward(self, x, *args, **kwargs):
        ...
        vit_features_output = self.vision_encoder_vit(x, *args, **kwargs)
        #
        # # ResNet分支增强流程 --------------------------
        # # 生成Gamma扰动图像
        # gamma = np.random.uniform(*self.iim.Gtheta)
        # x_gma = torch.pow(x, gamma).clamp(0, 1)
        #
        # # 通过IIM处理原始和扰动图像
        # x_enhanced, (feat_ii, feat_ii_gma) = self.iim(x)
        # _, (feat_ii_gma, _) = self.iim(x_gma)  # 扰动版本特征
        # print(x.shape)

        # input_tensor = enhance_x.half()
        # 仅将增强后的图像输入ResNet
        resnet_features = self.vision_encoder_resnet(x)
        # -------------------------------------------

        # # 打印 ResNet 输出的形状
        # for i, feature in enumerate(resnet_features):
        #     print(f"ResNet C{i + 1} shape: {feature.shape}")
        # return vit_features_output, resnet_features ,(feat_ii, feat_ii_gma)
        return vit_features_output, resnet_features
